...
function rand{T}(rng::AbstractRNG, g::RangeGeneratorInt{T,UInt64}, start::T)
    x = (g.k - 1) >> 32 == 0 ?
        rand_lessthan(rng, g.u % UInt32) % UInt64 :
        rand_lessthan(rng, g.u)         :: UInt64
    return start + rem_knuth(x, g.k) % T
end

rand{T}(rng::AbstractRNG, g::RangeGeneratorInt{T}, start::T) = start + rem_knuth(rand_lessthan(rng, g.u), g.k) % T
