...
@unix_only begin
    ...
    immutable RandomDevice <: AbstractRNG
        ...
        RandomDevice(unlimited=true) = new(open(unlimited ? "/dev/urandom" : "/dev/random"))
    end
...
@windows_only begin
    ...
    function rand{T<:Union(Bool, Base.IntTypes...)}(rd::RandomDevice, ::Type{T})
        win32_SystemFunction036!(pointer(rd.buffer), sizeof(T))
...
@inline rand(r::Union(MersenneTwister,RandomDevice), ::Type{Float64}) = rand(r, CloseOpen)
rand{T<:Union(Float16, Float32)}(r::Union(MersenneTwister,RandomDevice), ::Type{T}) = convert(T, rand(r, Float64))
